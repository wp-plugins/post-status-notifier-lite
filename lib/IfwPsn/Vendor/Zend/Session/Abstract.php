...
/**
 * IfwPsn_Vendor_Zend_Session_Abstract
 *
 * @category   Zend
 * @package    IfwPsn_Vendor_Zend_Session
 * @copyright  Copyright (c) 2005-2014 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwPsn_Vendor_Zend_Session_Abstract
{
    /**
     ...
     * but current IfwPsn_Vendor_Zend_Session has been marked as read-only.
     */
    const _THROW_NOT_WRITABLE_MSG = 'IfwPsn_Vendor_Zend_Session is currently marked as read-only.';


    /**
     * Error message thrown when an action requires reading session data,
     * but current IfwPsn_Vendor_Zend_Session is not marked as readable.
     */
    const _THROW_NOT_READABLE_MSG = 'IfwPsn_Vendor_Zend_Session is not marked as readable.';
    ...
    protected static function _namespaceUnset($namespace, $name = null)
    {
        if (self::$_writable === false) {
             ...
             */
            require_once IFW_PSN_LIB_ROOT . 'IfwPsn/Vendor/Zend/Session/Exception.php';
            throw new IfwPsn_Vendor_Zend_Session_Exception(self::_THROW_NOT_WRITABLE_MSG);
        }
