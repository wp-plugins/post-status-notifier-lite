...
class IfwPsn_Wp_Plugin_Update_Api_Envato extends IfwPsn_Wp_Plugin_Update_Api_Abstract
{
    ...
    public function getUpdateData($updateData)
    {
        ...
        if (!is_plugin_active($this->_pm->getSlugFilenamePath()) || !$this->_pm->isPremium() ||
            ((!property_exists($updateData, 'checked') || empty($updateData->checked)) && (int)$this->_pm->getConfig()->plugin->updateTest == 0)) {
            return $updateData;
        }
