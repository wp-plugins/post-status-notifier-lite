...
/**
 * IfwZend_Http_UserAgent_Features_Adapter_Interface
 */
//require_once 'IfwZend/Http/UserAgent/Features/Adapter.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Http_UserAgent_Features_Adapter_WurflApi
    implements IfwZend_Http_UserAgent_Features_Adapter
{
    ...
    public static function getFromRequest($request, array $config)
    {
        if (!isset($config['wurflapi'])) {
            //require_once 'IfwZend/Http/UserAgent/Features/Exception.php';
            throw new IfwZend_Http_UserAgent_Features_Exception('"wurflapi" configuration is not defined');
        }
        ...
        if (empty($config['wurfl_lib_dir'])) {
            //require_once 'IfwZend/Http/UserAgent/Features/Exception.php';
            throw new IfwZend_Http_UserAgent_Features_Exception('The "wurfl_lib_dir" parameter is not defined');
        }
        if (empty($config['wurfl_config_file']) && empty($config['wurfl_config_array'])) {
            //require_once 'IfwZend/Http/UserAgent/Features/Exception.php';
            throw new IfwZend_Http_UserAgent_Features_Exception('The "wurfl_config_file" parameter is not defined');
        }
        ...
        switch ($config['wurfl_api_version']) {
            case '1.0':
                // IfwZend_Http_UserAgent::$config['wurfl_config_file'] must be an XML file
                require_once ($config['wurfl_lib_dir'] . 'WURFLManagerProvider.php');
                $wurflManager = WURFL_WURFLManagerProvider::getWURFLManager(IfwZend_Http_UserAgent::$config['wurfl_config_file']);
                break;
            case '1.1':
            ...
            default:
                //require_once 'IfwZend/Http/UserAgent/Features/Exception.php';
                throw new IfwZend_Http_UserAgent_Features_Exception(sprintf(
