...
/**
 * @see IfwZend_Session_Abstract
 */
//require_once 'IfwZend/Session/Abstract.php';

/**
 * @see IfwZend_Session_Namespace
 */
//require_once 'IfwZend/Session/Namespace.php';

/**
 * @see IfwZend_Session_SaveHandler_Interface
 */
//require_once 'IfwZend/Session/SaveHandler/Interface.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Session extends IfwZend_Session_Abstract
{
    /**
     * Whether or not IfwZend_Session is being used with unit tests
     *
     * @internal
     * @var bool
     */
    ...
    /**
     * List of options pertaining to IfwZend_Session that can be set by developers
     * using IfwZend_Session::setOptions(). This list intentionally duplicates
    ...
    /**
     * Whether the default options listed in IfwZend_Session::$_localOptions have been set
     *
     * @var bool
     */
     ...
     * @throws IfwZend_Session_Exception
     * @return void
     */
    public static function setOptions(array $userOptions = array())
    {
        ...
        foreach ($userOptions as $userOptionName => $userOptionValue) {
            ...
            elseif (isset(self::$_localOptions[$userOptionName])) {
                ...
                /** @see IfwZend_Session_Exception */
                //require_once 'IfwZend/Session/Exception.php';
                throw new IfwZend_Session_Exception("Unknown option: $userOptionName = $userOptionValue");
            }
        }
    }
     ...
     *
     * @param IfwZend_Session_SaveHandler_Interface $interface
     * @return void
     */
    public static function setSaveHandler(IfwZend_Session_SaveHandler_Interface $saveHandler)
    {
    ...
    public static function regenerateId()
    {
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception("You must call " . __CLASS__ . '::' . __FUNCTION__ .
    ...
    public static function start($options = false)
    {
        if (self::$_sessionStarted && self::$_destroyed) {
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception('The session was explicitly destroyed during this request, attempting to re-start is not allowed.');
        }
        ...
        // In strict mode, do not allow auto-starting IfwZend_Session, such as via "new IfwZend_Session_Namespace()"
        if (self::$_strict && $options === true) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception('You must explicitly start the session with IfwZend_Session::start() when session options are set to strict.');
        }
        ...
        if (!self::$_unitTestEnabled && headers_sent($filename, $linenum)) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception("Session must be started before any output has been sent to the browser;"
        ...
        if (!self::$_unitTestEnabled && defined('SID')) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception('session has already been started by session.auto-start or session_start()');
        }
        ...
        if (!self::$_unitTestEnabled) {
            ...
            if (self::$_throwStartupExceptions) {
                //require_once 'IfwZend/Session/Exception.php';
                set_error_handler(array('IfwZend_Session_Exception', 'handleSessionStartError'), $errorLevel);
            }
            ...
            if (!$startedCleanly || IfwZend_Session_Exception::$sessionStartError != null) {
                if (self::$_throwStartupExceptions) {
                    set_error_handler(array('IfwZend_Session_Exception', 'handleSilentWriteClose'), $errorLevel);
                }
                session_write_close();
                if (self::$_throwStartupExceptions) {
                    restore_error_handler();
                    throw new IfwZend_Session_Exception(__CLASS__ . '::' . __FUNCTION__ . '() - ' . IfwZend_Session_Exception::$sessionStartError);
                }
            }
        }
     ...
     * has been called during this request by IfwZend_Session.
     *
     * @return bool
     */
    ...
    public static function setId($id)
    {
        if (!self::$_unitTestEnabled && defined('SID')) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception('The session has already been started.  The session id must be set first.');
        }
        ...
        if (!is_string($id) || $id === '') {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception('You must provide a non-empty string as a session identifier.');
        }
     ...
     *
     * @param IfwZend_Session_Validator_Interface $validator
     * @return void
     */
    public static function registerValidator(IfwZend_Session_Validator_Interface $validator)
    {
        $validator->setup();
    }
     ...
     *
     * @param bool $readonly - OPTIONAL remove write access (i.e. throw error if IfwZend_Session's attempt writes)
     * @return void
     */
    ...
    private static function _processValidators()
    {
        foreach ($_SESSION['__ZF']['VALID'] as $validator_name => $valid_data) {
            if (!class_exists($validator_name)) {
                ...
                Ifw_Zend_Loader::loadClass($validator_name);
            }
            $validator = new $validator_name;
            if ($validator->validate() === false) {
                /** @see IfwZend_Session_Exception */
                //require_once 'IfwZend/Session/Exception.php';
                throw new IfwZend_Session_Exception("This session is not valid according to {$validator_name}.");
            }
        }
    }
    ...
    public static function namespaceUnset($namespace)
    {
        ...
        IfwZend_Session_Namespace::resetSingleInstance($namespace);
    }


    /**
     * namespaceGet() - get all variables in a namespace
     * Deprecated: Use getIterator() in IfwZend_Session_Namespace.
     *
     * @param string $namespace
     * @return array
     */
    ...
    public static function getIterator()
    {
        if (parent::$_readable === false) {
            /** @see IfwZend_Session_Exception */
            //require_once 'IfwZend/Session/Exception.php';
            throw new IfwZend_Session_Exception(parent::_THROW_NOT_READABLE_MSG);
        }
