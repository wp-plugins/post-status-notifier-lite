...
/**
 * JSON Adapter for IfwZend_Config
 *
 * @category  Zend
 * @package   IfwZend_Config
 * @copyright Copyright (c) 2005-2009 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Config_Json extends IfwZend_Config
{
    /**
     ...
     * @throws IfwZend_Config_Exception When JSON text is not set or cannot be loaded
     * @throws IfwZend_Config_Exception When section $sectionName cannot be found in $json
     */
    public function __construct($json, $section = null, $options = false)
    {
        ...
        // Parse/decode
        $config = IfwZend_Json::decode($json);

        if (null === $config) {
            // decode failed
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception("Error parsing JSON data");
        }
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config[$sectionName])) {
                    //require_once 'IfwZend/Config/Exception.php';
                    throw new IfwZend_Config_Exception(sprintf('Section "%s" cannot be found', $sectionName));
                }
            ...
            if (!isset($config[$section])) {
                //require_once 'IfwZend/Config/Exception.php';
                throw new IfwZend_Config_Exception(sprintf('Section "%s" cannot be found', $section));
            }
    ...
    protected function _processExtends(array $data, $section, array $config = array())
    {
        ...
        if (is_array($thisSection) && isset($thisSection[self::EXTENDS_NAME])) {
            if (is_array($thisSection[self::EXTENDS_NAME])) {
                //require_once 'IfwZend/Config/Exception.php';
                throw new IfwZend_Config_Exception('Invalid extends clause: must be a string; array received');
            }
