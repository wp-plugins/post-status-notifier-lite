...
/**
 * @see IfwZend_Config_Writer
 */
//require_once 'IfwZend/Config/Writer/FileAbstract.php';

/**
 * @see IfwZend_Config_Yaml
 */
//require_once 'IfwZend/Config/Yaml.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Config_Writer_Yaml extends IfwZend_Config_Writer_FileAbstract
{
    /**
     ...
     * @var callable
     */
    protected $_yamlEncoder = array('IfwZend_Config_Writer_Yaml', 'encode');
     ...
     * @return IfwZend_Config_Yaml
     */
    public function setYamlEncoder($yamlEncoder)
    {
        if (!is_callable($yamlEncoder)) {
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception('Invalid parameter to setYamlEncoder - must be callable');
        }
    ...
    /**
     * Render a IfwZend_Config into a YAML config string.
     *
     * @since 1.10
     * @return string
     */
    public function render()
    {
        ...
        foreach ($extends as $section => $parentSection) {
            $data[$section][IfwZend_Config_Yaml::EXTENDS_NAME] = $parentSection;
        }

        // Ensure that each "extends" section actually exists
        foreach ($data as $section => $sectionData) {
            if (is_array($sectionData) && isset($sectionData[IfwZend_Config_Yaml::EXTENDS_NAME])) {
                $sectionExtends = $sectionData[IfwZend_Config_Yaml::EXTENDS_NAME];
                if (!isset($data[$sectionExtends])) {
                    // Remove "extends" declaration if section does not exist
                    unset($data[$section][IfwZend_Config_Yaml::EXTENDS_NAME]);
                }
            }
        }
