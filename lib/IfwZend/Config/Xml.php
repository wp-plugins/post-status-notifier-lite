...
/**
 * XML Adapter for IfwZend_Config
 *
 * @category  Zend
 * @package   IfwZend_Config
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Config_Xml extends IfwZend_Config
{
    /**
     ...
     * IfwZend_Config. If provided as an array, there are two configuration
     ...
     * @throws IfwZend_Config_Exception When xml is not set or cannot be loaded
     * @throws IfwZend_Config_Exception When section $sectionName cannot be found in $xml
     */
    public function __construct($xml, $section = null, $options = false)
    {
        if (empty($xml)) {
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception('Filename is not set');
        }
        ...
        if ($this->_loadFileErrorStr !== null) {
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception($this->_loadFileErrorStr);
        }
        ...
        if ($section === null) {
            ...
            foreach ($section as $sectionName) {
                if (!isset($config->$sectionName)) {
                    //require_once 'IfwZend/Config/Exception.php';
                    throw new IfwZend_Config_Exception("Section '$sectionName' cannot be found in $xml");
                }
            ...
            if (!isset($config->$section)) {
                //require_once 'IfwZend/Config/Exception.php';
                throw new IfwZend_Config_Exception("Section '$section' cannot be found in $xml");
            }
     ...
     * @throws IfwZend_Config_Exception When $section cannot be found
     * @return array
     */
    protected function _processExtends(SimpleXMLElement $element, $section, array $config = array())
    {
        if (!isset($element->$section)) {
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception("Section '$section' cannot be found");
        }
    ...
    protected function _toArray(SimpleXMLElement $xmlObject)
    {
        ...
        if (count($xmlObject->children(self::XML_NAMESPACE)) > 0) {
            if (count($xmlObject->children()) > 0) {
                //require_once 'IfwZend/Config/Exception.php';
                throw new IfwZend_Config_Exception("A node with a 'const' childnode may not have any other children");
            }
                ...
                switch ($node->localName) {
                    case 'const':
                        if (!$node->hasAttributeNS(self::XML_NAMESPACE, 'name')) {
                            //require_once 'IfwZend/Config/Exception.php';
                            throw new IfwZend_Config_Exception("Misssing 'name' attribute in 'const' node");
                        }
                        ...
                        if (!defined($constantName)) {
                            //require_once 'IfwZend/Config/Exception.php';
                            throw new IfwZend_Config_Exception("Constant with name '$constantName' was not defined");
                        }
                    ...
                    default:
                        //require_once 'IfwZend/Config/Exception.php';
                        throw new IfwZend_Config_Exception("Unknown node with name '$node->localName' found");
                }
            }
