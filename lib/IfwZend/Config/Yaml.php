...
/**
 * YAML Adapter for IfwZend_Config
 *
 * @category  Zend
 * @package   IfwZend_Config
 * @copyright Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license   http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Config_Yaml extends IfwZend_Config
{
    /**
    ...
    public function setYamlDecoder($yamlDecoder)
    {
        if (!is_callable($yamlDecoder)) {
            ...
            throw new IfwZend_Config_Exception('Invalid parameter to setYamlDecoder() - must be callable');
        }
    ...
    public function __construct($yaml, $section = null, $options = false)
    {
        ...
        if (null === $config) {
            // decode failed
            //require_once 'IfwZend/Config/Exception.php';
            throw new IfwZend_Config_Exception("Error parsing YAML data");
        }
    ...
    protected static function _decodeYaml($currentIndent, &$lines)
    {
        ...
        while (list($n, $line) = each($lines)) {
            ...
            if (preg_match("/(\w+):\s*(.*)/", $line, $m)) {
                ...
                //require_once 'IfwZend/Config/Exception.php';
                throw new IfwZend_Config_Exception(sprintf(
