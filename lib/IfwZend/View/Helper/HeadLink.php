...
/**
 * IfwZend_Layout_View_Helper_HeadLink
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_View_Helper_HeadLink extends IfwZend_View_Helper_Placeholder_Container_Standalone
{
    /**
     * $_validAttributes
     *
     ...
     */
    protected $_regKey = 'IfwZend_View_Helper_HeadLink';
     ...
     *
     * @return IfwZend_View_Helper_HeadLink
     */
    public function headLink(array $attributes = null, $placement = IfwZend_View_Helper_Placeholder_Container_Abstract::APPEND)
    {
        if (null !== $attributes) {
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(ap|pre)pend|offsetSet)(?P<type>Stylesheet|Alternate)$/', $method, $matches)) {
            ...
            if (1 > $argc) {
                ...
                $e =  new IfwZend_View_Exception(sprintf('%s requires at least one argument', $method));
                $e->setView($this->view);
                throw $e;
            }
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('append() expects a data token; please use one of the custom append*() methods');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function offsetSet($index, $value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('offsetSet() expects a data token; please use one of the custom offsetSet*() methods');
            $e->setView($this->view);
            throw $e;
        }
     ...
     *
     * @param  array $value
     * @return IfwZend_Layout_ViewHelper_HeadLink
     */
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('prepend() expects a data token; please use one of the custom prepend*() methods');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('set() expects a data token; please use one of the custom set*() methods');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function createDataAlternate(array $args)
    {
        if (3 > count($args)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception(sprintf('Alternate tags require 3 arguments; %s provided', count($args)));
            $e->setView($this->view);
            throw $e;
        }
