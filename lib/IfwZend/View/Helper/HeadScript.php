 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_View_Helper_HeadScript extends IfwZend_View_Helper_Placeholder_Container_Standalone
{
    /**#@+
     ...
     * @var string
     */
    protected $_regKey = 'IfwZend_View_Helper_HeadScript';
     ...
     * @return IfwZend_View_Helper_HeadScript
     */
    public function headScript($mode = IfwZend_View_Helper_HeadScript::FILE, $spec = null, $placement = 'APPEND', array $attrs = array(), $type = 'text/javascript')
    {
     ...
     * @return void
     */
    public function captureStart($captureType = IfwZend_View_Helper_Placeholder_Container_Abstract::APPEND, $type = 'text/javascript', $attrs = array())
    {
        if ($this->_captureLock) {
            //require_once 'IfwZend/View/Helper/Placeholder/Container/Exception.php';
            $e = new IfwZend_View_Helper_Placeholder_Container_Exception('Cannot nest headScript captures');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function captureEnd()
    {
        ...
        switch ($this->_captureType) {
            case IfwZend_View_Helper_Placeholder_Container_Abstract::SET:
            case IfwZend_View_Helper_Placeholder_Container_Abstract::PREPEND:
            case IfwZend_View_Helper_Placeholder_Container_Abstract::APPEND:
     ...
     * @param  array $args
     * @return IfwZend_View_Helper_HeadScript
     * @throws IfwZend_View_Exception if too few arguments or invalid method
     */
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(ap|pre)pend|offsetSet)(?P<mode>File|Script)$/', $method, $matches)) {
            if (1 > count($args)) {
                ...
                $e = new IfwZend_View_Exception(sprintf('Method "%s" requires at least one argument', $method));
                $e->setView($this->view);
                throw $e;
            }
            ...
            if ('offsetSet' == $action) {
                ...
                if (1 > count($args)) {
                    //require_once 'IfwZend/View/Exception.php';
                    $e = new IfwZend_View_Exception(sprintf('Method "%s" requires at least two arguments, an index and source', $method));
                    $e->setView($this->view);
                    throw $e;
                }
            }
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid argument passed to append(); please use one of the helper methods, appendScript() or appendFile()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid argument passed to prepend(); please use one of the helper methods, prependScript() or prependFile()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid argument passed to set(); please use one of the helper methods, setScript() or setFile()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function offsetSet($index, $value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid argument passed to offsetSet(); please use one of the helper methods, offsetSetScript() or offsetSetFile()');
            $e->setView($this->view);
            throw $e;
        }
