...
/**
 * IfwZend_Layout_View_Helper_HeadMeta
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_View_Helper_HeadMeta extends IfwZend_View_Helper_Placeholder_Container_Standalone
{
    /**
     * Types of attributes
     * @var array
     ...
     */
    protected $_regKey = 'IfwZend_View_Helper_HeadMeta';
     ...
     * @return IfwZend_View_Helper_HeadMeta
     */
    public function headMeta($content = null, $keyValue = null, $keyType = 'name', $modifiers = array(), $placement = IfwZend_View_Helper_Placeholder_Container_Abstract::APPEND)
    {
    ...
    protected function _normalizeType($type)
    {
        switch ($type) {
            ...
            default:
                ...
                $e = new IfwZend_View_Exception(sprintf('Invalid type "%s" passed to _normalizeType', $type));
                $e->setView($this->view);
                throw $e;
        }
    }
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(pre|ap)pend|offsetSet)(?P<type>Name|HttpEquiv|Property)$/', $method, $matches)) {
            ...
            if (2 > $argc) {
                //require_once 'IfwZend/View/Exception.php';
                $e = new IfwZend_View_Exception('Too few arguments provided; requires key value, and content');
                $e->setView($this->view);
                throw $e;
            }
	 ...
	 * @return IfwZend_View_Helper_HeadMeta Provides a fluent interface
	 */
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid value passed to append; please use appendMeta()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function offsetSet($index, $value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e =  new IfwZend_View_Exception('Invalid value passed to offsetSet; please use offsetSetName() or offsetSetHttpEquiv()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function offsetUnset($index)
    {
        if (!in_array($index, $this->getContainer()->getKeys())) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid index passed to offsetUnset()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid value passed to prepend; please use prependMeta()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Invalid value passed to set; please use setMeta()');
            $e->setView($this->view);
            throw $e;
        }
    ...
    public function itemToString(stdClass $item)
    {
        if (!in_array($item->type, $this->_typeKeys)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception(sprintf('Invalid type "%s" provided for meta', $item->type));
            $e->setView($this->view);
            throw $e;
        }
        $type = $item->type;
        ...
        foreach ($item->modifiers as $key => $value) {
            ...
            && $key == 'scheme') {
                //require_once 'IfwZend/View/Exception.php';
                throw new IfwZend_View_Exception('Invalid modifier '
        ...
        if ($this->view instanceof IfwZend_View_Abstract) {
            if ($this->view->doctype()->isHtml5()
            && $type == 'charset') {
