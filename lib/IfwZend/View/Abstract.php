...
/** @see IfwZend_Loader_PluginLoader */
//require_once 'IfwZend/Loader/PluginLoader.php';

/** @see IfwZend_View_Interface */
//require_once 'IfwZend/View/Interface.php';

/**
 * Abstract class for IfwZend_View to help enforce private constructs.
 *
 * @category   Zend
 * @package    IfwZend_View
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwZend_View_Abstract implements IfwZend_View_Interface
{
    /**
    ...
    /**
     * Stack of IfwZend_View_Filter names to apply as filters.
     * @var array
     */
    private $_filter = array();

    /**
     * Stack of IfwZend_View_Filter objects that have been loaded
     * @var array
     */
    ...
    /**
     * Allow custom object initialization when extending IfwZend_View_Abstract or
     * IfwZend_View
     *
     ...
     * @return void
     * @throws IfwZend_View_Exception if an attempt to set a private or protected
     * member is detected
     */
    public function __set($key, $val)
    {
        ...
        $e = new IfwZend_View_Exception('Setting private or protected class members is not allowed');
        $e->setView($this);
        throw $e;
    }
     ...
     * @return IfwZend_View_Abstract
     */
    public function setBasePath($path, $classPrefix = 'IfwZend_View')
    {
        $path        = rtrim($path, '/');
        $path        = rtrim($path, '\\');
     ...
     * @return IfwZend_View_Abstract
     */
    public function addBasePath($path, $classPrefix = 'IfwZend_View')
    {
        $path        = rtrim($path, '/');
        $path        = rtrim($path, '\\');
        $path       .= DIRECTORY_SEPARATOR;
     ...
     *
     * To clear all paths, use IfwZend_View::setScriptPath(null).
     *
     ...
     * @throws IfwZend_View_Exception if no script directory set
     */
    public function getScriptPath($name)
    {
        try {
            ...
            return $path;
        } catch (IfwZend_View_Exception $e) {
     ...
     *
     * @param  IfwZend_Loader_PluginLoader $loader
     * @param  string $type
     * @return IfwZend_View_Abstract
     */
    public function setPluginLoader(IfwZend_Loader_PluginLoader $loader, $type)
    {
        $type = strtolower($type);
        if (!in_array($type, $this->_loaderTypes)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception(sprintf('Invalid plugin loader type "%s"', $type));
            $e->setView($this);
            throw $e;
        }
    ...
    public function getPluginLoader($type)
    {
        ...
        if (!in_array($type, $this->_loaderTypes)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception(sprintf('Invalid plugin loader type "%s"; cannot retrieve', $type));
            $e->setView($this);
            throw $e;
        }

        if (!array_key_exists($type, $this->_loaders)) {
            $prefix     = 'IfwZend_View_';
            $pathPrefix = 'IfwZend/View/';
            ...
            switch ($type) {
                ...
                default:
                    $prefix     .= $pType;
                    $pathPrefix .= $pType;
                    $loader = new IfwZend_Loader_PluginLoader(array(
                        $prefix => $pathPrefix
                    ));
                    $this->_loaders[$type] = $loader;
                    break;
     ...
     * directory; defaults to IfwZend_View_Helper
     * @return IfwZend_View_Abstract
     */
    public function addHelperPath($path, $classPrefix = 'IfwZend_View_Helper_')
    {
     ...
     *
     * To clear all paths, use IfwZend_View::setHelperPath(null).
     *
     ...
     * $path; defaults to IfwZend_View_Helper
     * @return IfwZend_View_Abstract
     */
    public function setHelperPath($path, $classPrefix = 'IfwZend_View_Helper_')
    {
     ...
     *
     * @param  IfwZend_View_Helper_Abstract|object $helper
     * @param  string $name
     * @return IfwZend_View_Abstract
     * @throws IfwZend_View_Exception
     */
    public function registerHelper($helper, $name)
    {
        if (!is_object($helper)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('View helper must be an object');
            $e->setView($this);
            throw $e;
        }

        if (!$helper instanceof IfwZend_View_Interface) {
            if (!method_exists($helper, $name)) {
                //require_once 'IfwZend/View/Exception.php';
                $e =  new IfwZend_View_Exception(
                    'View helper must implement IfwZend_View_Interface or have a method matching the name provided'
                );
                $e->setView($this);
                throw $e;
            }
     ...
     * @param string|array The directory (-ies) to add.
     * @param string $classPrefix Class prefix to use with classes in this
     * directory; defaults to IfwZend_View_Filter
     * @return IfwZend_View_Abstract
     */
    public function addFilterPath($path, $classPrefix = 'IfwZend_View_Filter_')
    {
     ...
     *
     * To clear all paths, use IfwZend_View::setFilterPath(null).
     *
     * @param string|array The directory (-ies) to set as the path.
     * @param string $classPrefix The class prefix to apply to all elements in
     * $path; defaults to IfwZend_View_Filter
     * @return IfwZend_View_Abstract
     */
    public function setFilterPath($path, $classPrefix = 'IfwZend_View_Filter_')
    {
     ...
     *
     * To clear all filters, use IfwZend_View::setFilter(null).
     *
     ...
     *
     * IfwZend_View::assign('name', $value) assigns a variable called 'name'
     * with the corresponding $value.
     *
     * IfwZend_View::assign($array) assigns the array keys as variable
     ...
     * as the value.
     * @return IfwZend_View_Abstract Fluent interface
     * @throws IfwZend_View_Exception if $spec is neither a string nor an array,
    ...
    public function assign($spec, $value = null)
    {
        ...
        if (is_string($spec)) {
            ...
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('assign() expects a string or array, received ' . gettype($spec));
            $e->setView($this);
            throw $e;
        }
     ...
     *
     * Clears all variables assigned to IfwZend_View either via {@link assign()} or
     ...
     *
     * Use in conjunction with {@link IfwZend_View_Helper_DeclareVars the declareVars() helper}
    ...
    protected function _script($name)
    {
        if ($this->isLfiProtectionOn() && preg_match('#\.\.[\\\/]#', $name)) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('Requested scripts may not include parent directory traversal ("../", "..\\" notation)');
            $e->setView($this);
            throw $e;
        }
        ...
        if (0 == count($this->_path['script'])) {
            //require_once 'IfwZend/View/Exception.php';
            $e = new IfwZend_View_Exception('no view script directory set; unable to determine location for view script');
            $e->setView($this);
            throw $e;
        }
        ...
        $message = "script '$name' not found in path ("
                 ...
                 . ")";
        $e = new IfwZend_View_Exception($message);
        $e->setView($this);
        throw $e;
    }
     ...
     *
     * IfwZend_View::_addPath($type, 'dirname') adds one directory
     * to the path stack.
     *
     * IfwZend_View::_addPath($type, $array) adds one directory for
        ...
        switch ($type) {
            ...
            default:
                $this->_path[$type] = array(array(
                    'prefix' => 'IfwZend_View_' . ucfirst($type) . '_',
    ...
    private function _getPluginPath($type, $name)
    {
        ...
        try {
            $loader->load($name);
            return $loader->getClassPath($name);
        } catch (IfwZend_Loader_Exception $e) {
            return false;
        }
    }
