...
class IfwZend_Log
{
    ...
    /**
     * @var array of IfwZend_Log_Writer_Abstract
     */
     ...
     * @var string
     */
    protected $_defaultWriterNamespace = 'IfwZend_Log_Writer';

    /**
     *
     * @var string
     */
    protected $_defaultFilterNamespace = 'IfwZend_Log_Filter';

    /**
     *
     * @var string
     */
    protected $_defaultFormatterNamespace = 'IfwZend_Log_Formatter';
     ...
     *
     * @param IfwZend_Log_Writer_Abstract|null  $writer  default writer
     * @return void
     */
    public function __construct(IfwZend_Log_Writer_Abstract $writer = null)
    {
     ...
     *
     * @param  array|IfwZend_Config Array or instance of IfwZend_Config
     * @return IfwZend_Log
    ...
    protected function _constructWriterFromConfig($config)
    {
        ...
        if (!$writer instanceof IfwZend_Log_Writer_Abstract) {
            $writerName = is_object($writer)
                        ? get_class($writer)
                        : 'The specified writer';
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception("{$writerName} does not extend IfwZend_Log_Writer_Abstract!");
        }
    ...
    /**
     * Construct filter object from configuration array or IfwZend_Config object
     *
     * @param  array|IfwZend_Config $config IfwZend_Config or Array
     * @return IfwZend_Log_Filter_Interface
     * @throws IfwZend_Log_Exception
     */
    protected function _constructFilterFromConfig($config)
    {
        ...
        if (!$filter instanceof IfwZend_Log_Filter_Interface) {
            ...
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception("{$filterName} does not implement IfwZend_Log_Filter_Interface");
        }
   ...
   /**
     * Construct formatter object from configuration array or IfwZend_Config object
     *
     * @param  array|IfwZend_Config $config IfwZend_Config or Array
     * @return IfwZend_Log_Formatter_Interface
     * @throws IfwZend_Log_Exception
     */
    protected function _constructFormatterFromConfig($config)
    {
        ...
        if (!$formatter instanceof IfwZend_Log_Formatter_Interface) {
             $formatterName = is_object($formatter)
                         ? get_class($formatter)
                         : 'The specified formatter';
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception($formatterName . ' does not implement IfwZend_Log_Formatter_Interface');
        }
     ...
     * @param mixed $config IfwZend_Config or Array
    ...
    protected function _constructFromConfig($type, $config, $namespace)
    {
        ...
        if (!class_exists($className)) {
            ...
            Ifw_Zend_Loader::loadClass($className);
        }

        $reflection = new ReflectionClass($className);
        if (!$reflection->implementsInterface('IfwZend_Log_FactoryInterface')) {
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception(
                $className . ' does not implement IfwZend_Log_FactoryInterface and can not be constructed from config.'
            );
        }
    ...
    protected function getClassName($config, $type, $defaultNamespace)
    {
        if (!isset($config[ $type . 'Name' ])) {
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception("Specify {$type}Name in the configuration array");
        }
     ...
     *     instead of
     *   $log->log('message', IfwZend_Log::PRIORITY_NAME)
     *
        ...
        if (($priority = array_search($priority, $this->_priorities)) !== false) {
            switch (count($params)) {
                case 0:
                    /** @see IfwZend_Log_Exception */
                    //require_once 'IfwZend/Log/Exception.php';
                    throw new IfwZend_Log_Exception('Missing log message');
                case 1:
            ...
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception('Bad log priority');
        }
    }
    ...
    public function log($message, $priority, $extras = null)
    {
        ...
        if (empty($this->_writers)) {
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception('No writers were added');
        }
    ...
    public function addPriority($name, $priority)
    {
        ...
        if (isset($this->_priorities[$priority])
            ...
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception('Existing priorities cannot be overwritten');
        }
     ...
     *
     * @param  int|IfwZend_Config|array|IfwZend_Log_Filter_Interface $filter
     * @return IfwZend_Log
     * @throws IfwZend_Log_Exception
     */
    ...
    {
        if (is_int($filter)) {
            /** @see IfwZend_Log_Filter_Priority */
            //require_once 'IfwZend/Log/Filter/Priority.php';
            $filter = new IfwZend_Log_Filter_Priority($filter);

        } elseif ($filter instanceof IfwZend_Config || is_array($filter)) {
            $filter = $this->_constructFilterFromConfig($filter);

        } elseif(! $filter instanceof IfwZend_Log_Filter_Interface) {
            /** @see IfwZend_Log_Exception */
            //require_once 'IfwZend/Log/Exception.php';
     ...
     *
     * @param  mixed $writer IfwZend_Log_Writer_Abstract or Config array
     * @return IfwZend_Log
     */
    public function addWriter($writer)
    {
        if (is_array($writer) || $writer instanceof  IfwZend_Config) {
        ...
        if (!$writer instanceof IfwZend_Log_Writer_Abstract) {
            ...
            //require_once 'IfwZend/Log/Exception.php';
            throw new IfwZend_Log_Exception(
                'Writer must be an instance of IfwZend_Log_Writer_Abstract'
    ...
    public function registerErrorHandler()
    {
        ...
        // Contruct a default map of phpErrors to IfwZend_Log priorities.
        // Some of the errors are uncatchable, but are included for completeness
        $this->_errorHandlerMap = array(
            E_NOTICE            => IfwZend_Log::NOTICE,
            E_USER_NOTICE       => IfwZend_Log::NOTICE,
            E_WARNING           => IfwZend_Log::WARN,
            E_CORE_WARNING      => IfwZend_Log::WARN,
            E_USER_WARNING      => IfwZend_Log::WARN,
            E_ERROR             => IfwZend_Log::ERR,
            E_USER_ERROR        => IfwZend_Log::ERR,
            E_CORE_ERROR        => IfwZend_Log::ERR,
            E_RECOVERABLE_ERROR => IfwZend_Log::ERR,
            E_STRICT            => IfwZend_Log::DEBUG,
        );
        // PHP 5.3.0+
        if (defined('E_DEPRECATED')) {
            $this->_errorHandlerMap['E_DEPRECATED'] = IfwZend_Log::DEBUG;
        }
        if (defined('E_USER_DEPRECATED')) {
            $this->_errorHandlerMap['E_USER_DEPRECATED'] = IfwZend_Log::DEBUG;
        }
