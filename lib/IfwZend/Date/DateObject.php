 ...
 * @category   Zend
 * @package    IfwZend_Date
 * @subpackage IfwZend_Date_DateObject
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwZend_Date_DateObject {
    ...
    protected function setUnixTimestamp($timestamp = null)
    {
        ...
        if (is_numeric($timestamp)) {
            ...
            throw new IfwZend_Date_Exception('\'' . $timestamp . '\' is not a valid UNIX timestamp', 0, null, $timestamp);
        }
    ...
    /**
     * Internal mktime function used by IfwZend_Date.
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('IfwZend_DateObject_mkTime_' . $this->_offset . '_' . $year.$month.$day.'_'.$hour.$minute.$second . '_'.(int)$gmt, '-','_');
        ...
        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($date), $id, array('IfwZend_Date'));
            } else {
    ...
    /**
     * Internal mktime function used by IfwZend_Date for handling 64bit timestamps.
     *
    ...
    protected function date($format, $timestamp = null, $gmt = false)
    {
        ...
        $origstamp = $timestamp;
        if (isset(self::$_cache)) {
            $idstamp = strtr('IfwZend_DateObject_date_' . $this->_offset . '_'. $timestamp . '_'.(int)$gmt, '-','_');
        ...
        if (empty($gmt) and empty($jump)) {
            ...
            if (isset(self::$_cache)) {
                if (self::$_cacheTags) {
                    self::$_cache->save( serialize($timestamp), $idstamp, array('IfwZend_Date'));
                } else {
    ...
    protected function getDateParts($timestamp = null, $fast = null)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('IfwZend_DateObject_getDateParts_' . $timestamp.'_'.(int)$fast, '-','_');
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
        ...
        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($array), $id, array('IfwZend_Date'));
            } else {
     ...
     * @return IfwZend_Date_DateObject Provides fluent interface
     * @throws IfwZend_Date_Exception
     */
    public function setTimezone($zone = null)
    {
        ...
        if (function_exists('timezone_open')) {
            if (!@timezone_open($zone)) {
                //require_once 'IfwZend/Date/Exception.php';
                throw new IfwZend_Date_Exception("timezone ($zone) is not a known timezone", 0, null, $zone);
            }
        }
