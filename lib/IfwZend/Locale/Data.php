...
class IfwZend_Locale_Data
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new IfwZend_Locale_Exception("Missing locale file '$filename' for '$locale' locale.");
            }
     ...
     *
     * @param string|IfwZend_Locale $locale Locale to check
     * @return string
     */
    private static function _checkLocale($locale)
    {
        ...
        if (!(IfwZend_Locale::isLocale((string) $locale, null, false))) {
            //require_once 'IfwZend/Locale/Exception.php';
            throw new IfwZend_Locale_Exception("Locale (" . (string) $locale . ") is a unknown locale");
        }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (!isset(self::$_cache) && !self::$_cacheDisabled) {
            ...
            self::$_cache = IfwZend_Cache::factory(
                'Core',
                'File',
        ...
        $val = urlencode($val);
        $id = strtr('IfwZend_LocaleL_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        ...
        switch(strtolower($path)) {
            ...
            default :
                //require_once 'IfwZend/Locale/Exception.php';
                throw new IfwZend_Locale_Exception("Unknown list ($path) for parsing locale data.");
                break;
        }

        if (isset(self::$_cache)) {
            if (self::$_cacheTags) {
                self::$_cache->save( serialize($temp), $id, array('IfwZend_Locale'));
            } else {
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        $val = urlencode($val);
        $id = strtr('IfwZend_LocaleC_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        if (!self::$_cacheDisabled && ($result = self::$_cache->load($id))) {
            return unserialize($result);
        }
        ...
        switch(strtolower($path)) {
            ...
            default :
                //require_once 'IfwZend/Locale/Exception.php';
                throw new IfwZend_Locale_Exception("Unknown detail ($path) for parsing locale data.");
                break;
        }
    ...
    /**
     * Set a cache for IfwZend_Locale_Data
     *
     * @param IfwZend_Cache_Core $cache A cache frontend
     */
    ...
    {
        if (self::$_cacheTags) {
            self::$_cache->clean(IfwZend_Cache::CLEANING_MODE_MATCHING_TAG, array('IfwZend_Locale'));
        } else {
