 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Application_Resource_Mail extends IfwZend_Application_Resource_ResourceAbstract
{
    ...
    /**
     *
     * @return IfwZend_Mail_Transport_Abstract|null
     */
    public function getMail()
    {
        if (null === $this->_transport) {
               ...
               !is_numeric($options['transport']))
            {
                ...
                {
                    IfwZend_Mail::setDefaultTransport($this->_transport);
                }
            }
    ...
    protected function _setDefaults($type) {
        ...
        {
            $method = array('IfwZend_Mail', 'setDefault' . ucfirst($type));
    ...
    protected function _setupTransport($options)
    {
        ...
        if(!IfwZend_Loader_Autoloader::autoload($transportName))
        {
            $transportName = ucfirst(strtolower($transportName));

            if(!IfwZend_Loader_Autoloader::autoload($transportName))
            {
                $transportName = 'IfwZend_Mail_Transport_' . $transportName;
                if(!IfwZend_Loader_Autoloader::autoload($transportName)) {
        ...
        switch($transportName) {
            case 'IfwZend_Mail_Transport_Smtp':
                ...
                break;
            case 'IfwZend_Mail_Transport_Sendmail':
            default:
