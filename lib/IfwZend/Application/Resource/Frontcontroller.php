 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Application_Resource_Frontcontroller extends IfwZend_Application_Resource_ResourceAbstract
{
    /**
                ...
                case 'actionhelperpaths':
                    if (is_array($value)) {
                        foreach ($value as $helperPrefix => $helperPath) {
                            IfwZend_Controller_Action_HelperBroker::addPath($helperPath, $helperPrefix);
                        }
                    }
                    break;
    ...
    {
        if (null === $this->_front) {
            $this->_front = IfwZend_Controller_Front::getInstance();
        }
        return $this->_front;
    }
}
