 ...
 *
 * @uses       IfwZend_Application_Resource_Base
 * @category   Zend
 * @package    IfwZend_Application
 * @subpackage Resource
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Application_Resource_Locale
    extends IfwZend_Application_Resource_ResourceAbstract
{
    const DEFAULT_REGISTRY_KEY = 'IfwZend_Locale';
    ...
    public function getLocale()
    {
        if (null === $this->_locale) {
            ...
            if (!isset($options['default'])) {
                $this->_locale = new IfwZend_Locale();
            } elseif(!isset($options['force']) ||
                     (bool) $options['force'] == false)
            {
                // Don't force any locale, just go for auto detection
                IfwZend_Locale::setDefault($options['default']);
                $this->_locale = new IfwZend_Locale();
            } else {
                $this->_locale = new IfwZend_Locale($options['default']);
            }

            $key = (isset($options['registry_key']) && !is_numeric($options['registry_key']))
                ? $options['registry_key']
                : self::DEFAULT_REGISTRY_KEY;
            IfwZend_Registry::set($key, $this->_locale);
        }
     ...
     * @return IfwZend_Application_Resource_Locale
     */
    public function setCache($cache)
    {
        ...
        if ($cache instanceof IfwZend_Cache_Core) {
            IfwZend_Locale::setCache($cache);
        }
