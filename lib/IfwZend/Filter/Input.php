...
/**
 * @see IfwZend_Validate
 */
//require_once 'IfwZend/Validate.php';
...
class IfwZend_Filter_Input
{
    ...
    /**
     * @var IfwZend_Filter_Interface The filter object that is run on values
     ...
     * @return IfwZend_Filter_Input
     * @throws IfwZend_Filter_Exception on invalid type
     */
    public function setPluginLoader(IfwZend_Loader_PluginLoader_Interface $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new IfwZend_Filter_Exception(sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    ...
    public function getPluginLoader($type)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                case self::FILTER:
                    $prefixSegment = 'IfwZend_Filter_';
                    $pathSegment   = 'IfwZend/Filter/';
                    break;
                case self::VALIDATE:
                    $prefixSegment = 'IfwZend_Validate_';
                    $pathSegment   = 'IfwZend/Validate/';
                    break;
                default:
                    //require_once 'IfwZend/Filter/Exception.php';
                    throw new IfwZend_Filter_Exception(sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }
    ...
    /**
     * @return IfwZend_Filter_Input
     * @throws IfwZend_Filter_Exception
     */
    public function process()
    {
        ...
        if ($this->hasInvalid()) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception("Input has invalid fields");
        }
        if ($this->hasMissing()) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception("Input has missing fields");
        }
    ...
    public function setDefaultEscapeFilter($escapeFilter)
    {
        ...
        if (!$escapeFilter instanceof IfwZend_Filter_Interface) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('Escape filter specified does not implement IfwZend_Filter_Interface');
        }
     ...
     * @param array $options
     * @return IfwZend_Filter_Input
     * @throws IfwZend_Filter_Exception if an unknown option is given
     */
    public function setOptions(array $options)
    {
        foreach ($options as $option => $value) {
            switch ($option) {
                ...
                default:
                    //require_once 'IfwZend/Filter/Exception.php';
                    throw new IfwZend_Filter_Exception("Unknown option '$option'");
                    break;
            }
        }
    ...
    {
        if ((null === $translator) || ($translator instanceof IfwZend_Translate_Adapter)) {
            $this->_translator = $translator;
        } elseif ($translator instanceof IfwZend_Translate) {
            $this->_translator = $translator->getAdapter();
        } else {
            //require_once 'IfwZend/Validate/Exception.php';
            throw new IfwZend_Validate_Exception('Invalid translator specified');
        }
    ...
    protected function _filter()
    {
        foreach ($this->_filterRules as $ruleName => &$filterRule) {
             ...
             * Don't typecast to (array) because it might be a IfwZend_Filter object
             */
            ...
            if (!isset($filterRule[self::FILTER_CHAIN])) {
                $filterRule[self::FILTER_CHAIN] = new IfwZend_Filter();
    ...
    protected function _validate()
    {
        ...
        foreach ($this->_validatorRules as $ruleName => &$validatorRule) {
             ...
             * Don't typecast to (array) because it might be a IfwZend_Validate object
             */
            ...
            if (!isset($validatorRule[self::ALLOW_EMPTY])) {
                ...
                foreach ($validatorRule as $rule) {
                    ...
                    if($rule instanceof IfwZend_Validate_NotEmpty) {
                        $foundNotEmptyValidator = true;
                        // field may not be empty, we are ready
                        break 1;
                    }
            ...
            if (!isset($validatorRule[self::VALIDATOR_CHAIN])) {
                $validatorRule[self::VALIDATOR_CHAIN] = new IfwZend_Validate();
                ...
                foreach ($validatorList as $key => $validator) {
                    ...
                    if (isset($validatorRule[self::MESSAGES][$key])) {
                        ...
                        if ($validator instanceof IfwZend_Validate_NotEmpty) {
        ...
        if (count((array) $validatorRule[self::FIELDS]) > 1) {
            if (!$validatorRule[self::ALLOW_EMPTY]) {
                ...
                foreach ($data as $fieldKey => $field) {
                    // if there is no IfwZend_Validate_NotEmpty instance in the rules, we will use the default
            ...
            if ($validatorRule[self::ALLOW_EMPTY]) {
                ...
                $validatorChain = new IfwZend_Validate();
     ...
     * set on the validator outside IfwZend_Filter_Input.
     * @param array $validatorRule
     * @return mixed false if none is found, IfwZend_Validate_NotEmpty instance if found
     */
    protected function _getNotEmptyValidatorInstance($validatorRule) {
        foreach ($validatorRule as $rule => $value) {
            if (is_object($value) and $value instanceof IfwZend_Validate_NotEmpty) {
                return $value;
            }
        }
     ...
     * @param string $type
     * @param mixed $classBaseName
     * @return IfwZend_Filter_Interface|IfwZend_Validate_Interface
     * @throws IfwZend_Filter_Exception
     */
    protected function _getFilterOrValidator($type, $classBaseName)
    {
        ...
        $interfaceName = 'IfwZend_' . ucfirst($type) . '_Interface';
        ...
        if (!$class->implementsInterface($interfaceName)) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception("Class '$className' based on basename '$classBaseName' must implement the '$interfaceName' interface");
        }
