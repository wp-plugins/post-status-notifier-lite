 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Filter_Inflector implements IfwZend_Filter_Interface
{
    /**
    ...
    {
        if (!$this->_pluginLoader instanceof IfwZend_Loader_PluginLoader_Interface) {
            $this->_pluginLoader = new IfwZend_Loader_PluginLoader(array('IfwZend_Filter_' => 'IfwZend/Filter/'), __CLASS__);
        }
     ...
     * Set PluginLoader
     *
     * @param IfwZend_Loader_PluginLoader_Interface $pluginLoader
     * @return IfwZend_Filter_Inflector
     */
    public function setPluginLoader(IfwZend_Loader_PluginLoader_Interface $pluginLoader)
    {
    ...
    /**
     * Use IfwZend_Config object to set object state
     *
     ...
     * @param string $spec
     * @param int $index
     * @return IfwZend_Filter_Interface|false
     */
     ...
     *
     * @param string $spec
     * @param array|string|IfwZend_Filter_Interface $ruleSet
     * @return IfwZend_Filter_Inflector
     */
    ...
    public function filter($source)
    {
        ...
        if ($this->_throwTargetExceptionsOn && (preg_match('#(?='.$pregQuotedTargetReplacementIdentifier.'[A-Za-z]{1})#', $inflectedTarget) == true)) {
            ...
            throw new IfwZend_Filter_Exception('A replacement identifier ' . $this->_targetReplacementIdentifier . ' was found inside the inflected target, perhaps a rule was not satisfied with a target source?  Unsatisfied inflected target: ' . $inflectedTarget);
        }
    ...
    {
        if ($rule instanceof IfwZend_Filter_Interface) {
            return $rule;
        }
        ...
        if (!$ruleObject instanceof IfwZend_Filter_Interface) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('No class named ' . $rule . ' implementing IfwZend_Filter_Interface could be found');
        }
