 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Filter_Compress_Gz extends IfwZend_Filter_Compress_CompressAbstract
{
    /**
     * Compression Options
     * array(
     ...
     * Class constructor
     *
     * @param array|IfwZend_Config|null $options (Optional) Options to set
     */
    public function __construct($options = null)
    {
        if (!extension_loaded('zlib')) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('This filter needs the zlib extension');
        }
     ...
     *
     * @param integer $level
     * @return IfwZend_Filter_Compress_Gz
     */
    public function setLevel($level)
    {
        if (($level < 0) || ($level > 9)) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('Level must be between 0 and 9');
        }
    ...
    public function setMode($mode)
    {
        if (($mode != 'compress') && ($mode != 'deflate')) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('Given compression mode not supported');
        }
    ...
    public function compress($content)
    {
        ...
        if (!empty($archive)) {
            ...
            if (!$file) {
                //require_once 'IfwZend/Filter/Exception.php';
                throw new IfwZend_Filter_Exception("Error opening the archive '" . $this->_options['archive'] . "'");
            }
        ...
        if (!$compressed) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('Error during compression');
        }
    ...
    public function decompress($content)
    {
        ...
        if (file_exists($archive)) {
            ...
            if (!$handler) {
                //require_once 'IfwZend/Filter/Exception.php';
                throw new IfwZend_Filter_Exception("Error opening the archive '" . $archive . "'");
            }
