 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Filter_LocalizedToNormalized implements IfwZend_Filter_Interface
{
    /**
     * Set options
     * @var array
    ...
    {
        if (IfwZend_Locale_Format::isNumber($value, $this->_options)) {
            return IfwZend_Locale_Format::getNumber($value, $this->_options);
        } else if (($this->_options['date_format'] === null) && (strpos($value, ':') !== false)) {
            // Special case, no date format specified, detect time input
            return IfwZend_Locale_Format::getTime($value, $this->_options);
        } else if (IfwZend_Locale_Format::checkDateFormat($value, $this->_options)) {
            // Detect date or time input
            return IfwZend_Locale_Format::getDate($value, $this->_options);
        }
