 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Filter_Boolean implements IfwZend_Filter_Interface
{
    const BOOLEAN      = 1;
    const INTEGER      = 2;
    const FLOAT        = 4;
     ...
     * Constructor
     *
     * @param string|array|IfwZend_Config $options OPTIONAL
     */
    ...
    public function setType($type = null)
    {
        ...
        if (!is_int($type) || ($type < 0) || ($type > self::ALL)) {
            ...
            throw new IfwZend_Filter_Exception('Unknown type');
        }
     ...
     *
     * @param  string|array|IfwZend_Locale $locale
     * @throws IfwZend_Filter_Exception
     * @return IfwZend_Filter_Boolean
     */
    public function setLocale($locale = null)
    {
        if (is_string($locale)) {
            $locale = array($locale);
        } elseif ($locale instanceof IfwZend_Locale) {
            $locale = array($locale->toString());
        } elseif (!is_array($locale)) {
            //require_once 'IfwZend/Filter/Exception.php';
            throw new IfwZend_Filter_Exception('Locale has to be string, array or an instance of IfwZend_Locale');
        }

        //require_once 'IfwZend/Locale.php';
        foreach ($locale as $single) {
            if (!IfwZend_Locale::isLocale($single)) {
                //require_once 'IfwZend/Filter/Exception.php';
                throw new IfwZend_Filter_Exception("Unknown locale '$single'");
            }
        }
    ...
    protected function _getLocalizedQuestion($value, $yes, $locale)
    {
        ...
        $str = IfwZend_Locale::getTranslation($question, 'question', $locale);
        $str = explode(':', $str);
        if (!empty($str)) {
            foreach($str as $no) {
