 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwZend_Mail_Transport_Abstract
{
    /**
     * Mail body
     * @var string
     ...
     * on the mail parts present in the {@link $_mail IfwZend_Mail object} present.
     *
        ...
        if (null !== $boundary) {
            ...
            if (!$type) {
                if ($this->_mail->hasAttachments) {
                    $type = IfwZend_Mime::MULTIPART_MIXED;
                } elseif ($this->_mail->getBodyText() && $this->_mail->getBodyHtml()) {
                    $type = IfwZend_Mime::MULTIPART_ALTERNATIVE;
                } else {
                    $type = IfwZend_Mime::MULTIPART_MIXED;
                }
            }
     ...
     * @access protected
     * @return void
     * @throws IfwZend_Mail_Transport_Exception if any header lines exceed 998
     * characters
     */
    protected function _prepareHeaders($headers)
    {
        if (!$this->_mail) {
            /**
             * @see IfwZend_Mail_Transport_Exception
             */
            //require_once 'IfwZend/Mail/Transport/Exception.php';
            throw new IfwZend_Mail_Transport_Exception('Missing IfwZend_Mail object in _mail property');
        }
        ...
        if (!$sane) {
             ...
             */
            //require_once 'IfwZend/Mail/Transport/Exception.php';
            throw new IfwZend_Mail_Exception('At least one mail header line is too long');
        }
    }
     ...
     * multipart/alternative IfwZend_Mime_Part containing the headers and contents
     * of each. Otherwise, uses whichever of the text or HTML parts present.
     *
     * The content part is then prepended to the list of IfwZend_Mime_Parts for
     * this message.
     *
     * @return void
     */
    protected function _buildBody()
    {
            ...
            && ($html = $this->_mail->getBodyHtml()))
        {
            ...
            $mp->type     = IfwZend_Mime::MULTIPART_ALTERNATIVE;
        ...
        if (!$body) {
             ...
             */
            //require_once 'IfwZend/Mail/Transport/Exception.php';
            throw new IfwZend_Mail_Transport_Exception('No body specified');
        }
        ...
        foreach ($headers as $header) {
            // Headers in IfwZend_Mime_Part are kept as arrays with two elements, a
            // key and a value
     ...
     *
     * @param  IfwZend_Mail $mail
     * @access public
     * @return void
     * @throws IfwZend_Mail_Transport_Exception if mail is empty
     */
    public function send(IfwZend_Mail $mail)
    {
        ...
        if ($count < 1) {
             ...
             */
            //require_once 'IfwZend/Mail/Transport/Exception.php';
            throw new IfwZend_Mail_Transport_Exception('Empty mail cannot be sent');
        }

        if ($count > 1) {
            // Multipart message; create new MIME object and boundary
            $mime     = new IfwZend_Mime($this->_mail->getMimeBoundary());
        ...
        // Create message body
        // This is done so that the same IfwZend_Mail object can be used in
        // multiple transports
        $message = new IfwZend_Mime_Message();
