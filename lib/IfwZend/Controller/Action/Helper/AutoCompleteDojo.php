 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Controller_Action_Helper_AutoCompleteDojo extends IfwZend_Controller_Action_Helper_AutoComplete_Abstract
{
    /**
    ...
    {
        if (!$data instanceof IfwZend_Dojo_Data) {
            //require_once 'IfwZend/Dojo/Data.php';
            $items = array();
            foreach ($data as $key => $value) {
                $items[] = array('label' => $value, 'name' => $value);
            }
            $data = new IfwZend_Dojo_Data('name', $items);
        }
