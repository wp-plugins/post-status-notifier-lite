 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Controller_Plugin_Broker extends IfwZend_Controller_Plugin_Abstract
{

    /**
     * Array of instance of objects extending IfwZend_Controller_Plugin_Abstract
     *
     * @var array
     */
     ...
     * @return IfwZend_Controller_Plugin_Broker
     */
    public function registerPlugin(IfwZend_Controller_Plugin_Abstract $plugin, $stackIndex = null)
    {
        if (false !== array_search($plugin, $this->_plugins, true)) {
            ...
            throw new IfwZend_Controller_Exception('Plugin already registered');
        }
        ...
        if ($stackIndex) {
            if (isset($this->_plugins[$stackIndex])) {
                //require_once 'IfwZend/Controller/Exception.php';
                throw new IfwZend_Controller_Exception('Plugin with stackIndex "' . $stackIndex . '" already registered');
            }
     ...
     * Unregister a plugin.
     *
     * @param string|IfwZend_Controller_Plugin_Abstract $plugin Plugin object or class name
     * @return IfwZend_Controller_Plugin_Broker
     */
    public function unregisterPlugin($plugin)
    {
        if ($plugin instanceof IfwZend_Controller_Plugin_Abstract) {
            // Given a plugin object, find it in the array
            $key = array_search($plugin, $this->_plugins, true);
            if (false === $key) {
                //require_once 'IfwZend/Controller/Exception.php';
                throw new IfwZend_Controller_Exception('Plugin never registered.');
            }
     ...
     * @return false|IfwZend_Controller_Plugin_Abstract|array Returns false if none found, plugin if only one found, and array of plugins if multiple plugins of same class found
     */
    ...
    public function routeStartup(IfwZend_Controller_Request_Abstract $request)
    {
        foreach ($this->_plugins as $plugin) {
            try {
                ...
                if (IfwZend_Controller_Front::getInstance()->throwExceptions()) {
                    throw new IfwZend_Controller_Exception($e->getMessage() . $e->getTraceAsString(), $e->getCode(), $e);
                } else {
    ...
    /**
     * Called before IfwZend_Controller_Front exits its iterations over
     * the route set.
     *
     * @param  IfwZend_Controller_Request_Abstract $request
     * @return void
     ...
     *
     * During the dispatch loop, IfwZend_Controller_Front keeps a
     * IfwZend_Controller_Request_Abstract object, and uses
     * IfwZend_Controller_Dispatcher to dispatch the
     * IfwZend_Controller_Request_Abstract object to controllers/actions.
     *
     * @param  IfwZend_Controller_Request_Abstract $request
     * @return void
     */
