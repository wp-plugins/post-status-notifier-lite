 ...
 * @version    $Id$
 */
class IfwZend_Controller_Plugin_ErrorHandler extends IfwZend_Controller_Plugin_Abstract
{
    /**
     ...
     *
     * @param  array $options
     * @return IfwZend_Controller_Plugin_ErrorHandler
     */
    ...
    public function getErrorHandlerModule()
    {
        if (null === $this->_errorModule) {
            $this->_errorModule = IfwZend_Controller_Front::getInstance()->getDispatcher()->getDefaultModule();
        }
     ...
     * @return void
     */
    protected function _handleError(IfwZend_Controller_Request_Abstract $request)
    {
        ...
        if (($response->isException()) && (!$this->_isInsideErrorHandlerLoop)) {
            ...
            switch ($exceptionType) {
                case 'IfwZend_Controller_Router_Exception':
                    ...
                    break;
                case 'IfwZend_Controller_Dispatcher_Exception':
                    $error->type = self::EXCEPTION_NO_CONTROLLER;
                    break;
                case 'IfwZend_Controller_Action_Exception':
                    if (404 == $exception->getCode()) {
