 ...
 * @subpackage IfwZend_Translate_Adapter
...
/**
 * @see IfwZend_Translate_Plural
 */
//require_once 'IfwZend/Translate/Plural.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class IfwZend_Translate_Adapter {
    /**
     ...
     *   'log'             => a instance of IfwZend_Log where logs are written to
     ...
     *
     * @param  array|IfwZend_Config $options Translation options for this adapter
    ...
    public function __construct($options = array())
    {
        ...
        if (isset(self::$_cache)) {
            $id = 'IfwZend_Translate_' . $this->toString() . '_Options';
     ...
     *
     * @param  array|IfwZend_Config $options Options and translations to be added
     * @throws IfwZend_Translate_Exception
     * @return IfwZend_Translate_Adapter Provides fluent interface
     */
    public function addTranslation($options = array())
    {
        ...
        if (!isset($options['content']) || empty($options['content'])) {
            ...
            throw new IfwZend_Translate_Exception("Required option 'content' is missing");
        }
        ...
        if ((array_key_exists('log', $options)) && !($options['log'] instanceof IfwZend_Log)) {
            //require_once 'IfwZend/Translate/Exception.php';
            throw new IfwZend_Translate_Exception('Instance of IfwZend_Log expected for option log');
        }

        try {
            if (!($options['content'] instanceof IfwZend_Translate) && !($options['content'] instanceof IfwZend_Translate_Adapter)) {
                if (empty($options['locale'])) {
                    $options['locale'] = null;
                }

                $options['locale'] = IfwZend_Locale::findLocale($options['locale']);
            }
        } catch (IfwZend_Locale_Exception $e) {
            //require_once 'IfwZend/Translate/Exception.php';
            throw new IfwZend_Translate_Exception("The given Language '{$options['locale']}' does not exist", 0, $e);
        }
        ...
        if (is_string($options['content']) and is_dir($options['content'])) {
            ...
            foreach ($iterator as $directory => $info) {
                ...
                if ($info->isDir()) {
                    // pathname as locale
                    if (($options['scan'] === self::LOCALE_DIRECTORY) and (IfwZend_Locale::isLocale($file, true, false))) {
                        $options['locale'] = $file;
                    ...
                    if ($options['scan'] === self::LOCALE_FILENAME) {
                        ...
                        if (IfwZend_Locale::isLocale((string) $filename, true, false)) {
                            ...
                            $prev  = '';
                            foreach($parts as $token) {
                                if (IfwZend_Locale::isLocale($token, true, false)) {
                    ...
                    } catch (IfwZend_Translate_Exception $e) {
    ...
    public function setOptions(array $options = array())
    {
        ...
        foreach ($options as $key => $option) {
            if ($key == 'locale') {
                ...
                if (($key == 'log') && !($option instanceof IfwZend_Log)) {
                    //require_once 'IfwZend/Translate/Exception.php';
                    throw new IfwZend_Translate_Exception('Instance of IfwZend_Log expected for option log');
                }
     ...
     * Gets locale
     *
     * @return IfwZend_Locale|string|null
     */
    ...
    public function setLocale($locale)
    {
        ...
        try {
            ...
            //require_once 'IfwZend/Translate/Exception.php';
            throw new IfwZend_Translate_Exception("The given Language ({$locale}) does not exist", 0, $e);
        }
     ...
     * @param  string|IfwZend_Locale $locale (optional) Language to return the message ids from
     ...
     *
     * @param  string|IfwZend_Locale $locale (optional) Language to return the messages from
     * @return array
     */
     ...
     *
     * @see    IfwZend_Locale
     * @param  string|IfwZend_Locale $locale Language to search for, identical with locale identifier,
     ...
     *
     * @see    IfwZend_Locale
     * @param  array|IfwZend_Config $content Translation data to add
     * @throws IfwZend_Translate_Exception
     * @return IfwZend_Translate_Adapter Provides fluent interface
     */
    private function _addTranslationData($options = array())
    {
        ...
        $read = true;
        if (isset(self::$_cache)) {
            $id = 'IfwZend_Translate_' . md5(serialize($options['content'])) . '_' . $this->toString();
        ...
        if ($this->_automatic === true) {
            $find = new IfwZend_Locale($options['locale']);
     ...
     * @param  string|IfwZend_Locale $locale    (optional) Locale/Language to use, identical with
     *                                       locale identifier, @see IfwZend_Locale for more information
     * @return string
     */
    public function translate($messageId, $locale = null)
    {
        ...
        if (!IfwZend_Locale::isLocale($locale, true, false)) {
            if (!IfwZend_Locale::isLocale($locale, false, false)) {
                ...
                $rule = IfwZend_Translate_Plural::getPlural($number, $plocale);
        ...
        if ((is_string($messageId) || is_int($messageId)) && isset($this->_translate[$locale][$messageId])) {
            ...
            $rule = IfwZend_Translate_Plural::getPlural($number, $locale);
     ...
     * @param  string|IfwZend_Locale $locale    (optional) Locale/Language to use, identical with locale
     *                                       identifier, @see IfwZend_Locale for more information
     * @return string
     */
     ...
     * @param  string|IfwZend_Locale $locale    (optional) Locale/Language to use, identical with locale identifier,
     *                                       see IfwZend_Locale for more information
     * @return boolean
     */
    ...
    /**
     * Sets a cache for all IfwZend_Translate_Adapters
     *
    ...
    {
        //require_once 'IfwZend/Cache.php';
        if (self::$_cacheTags) {
            if ($tag == null) {
                $tag = 'IfwZend_Translate';
            }

            self::$_cache->clean(IfwZend_Cache::CLEANING_MODE_MATCHING_TAG, array($tag));
        } else {
            self::$_cache->clean(IfwZend_Cache::CLEANING_MODE_ALL);
        }
    }
     ...
     *
     * @param IfwZend_Cache $cache
     */
    private static function _getTagSupportForCache()
    {
        $backend = self::$_cache->getBackend();
        if ($backend instanceof IfwZend_Cache_Backend_ExtendedInterface) {
