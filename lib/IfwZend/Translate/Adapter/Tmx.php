 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Translate_Adapter_Tmx extends IfwZend_Translate_Adapter {
    // Internal variables
    private $_file    = false;
    private $_useId   = true;
     ...
     * @throws IfwZend_Translation_Exception
     * @return array
     */
    protected function _loadTranslationData($filename, $locale, array $options = array())
    {
        ...
        if (!is_readable($filename)) {
            ...
            throw new IfwZend_Translate_Exception('Translation file \'' . $filename . '\' is not readable.');
        }
        ...
        if (!xml_parse($this->_file, file_get_contents($filename))) {
            ...
            //require_once 'IfwZend/Translate/Exception.php';
            throw new IfwZend_Translate_Exception($ex);
        }
            ...
            switch(strtolower($name)) {
                case 'header':
                    if (empty($this->_useId) && isset($attrib['srclang'])) {
                        if (IfwZend_Locale::isLocale($attrib['srclang'])) {
                            $this->_srclang = IfwZend_Locale::findLocale($attrib['srclang']);
                        } else {
                ...
                case 'tuv':
                    if (isset($attrib['xml:lang'])) {
                        if (IfwZend_Locale::isLocale($attrib['xml:lang'])) {
                            $this->_tuv = IfwZend_Locale::findLocale($attrib['xml:lang']);
                        } else {
                            if (!$this->_options['disableNotices']) {
                                if ($this->_options['log']) {
