 ...
 * @subpackage IfwZend_Cache_Backend
...
/**
 * @see IfwZend_Cache_Backend_ExtendedInterface
 */
//require_once 'IfwZend/Cache/Backend/ExtendedInterface.php';

/**
 * @see IfwZend_Cache_Backend
 */
//require_once 'IfwZend/Cache/Backend.php';
...
class IfwZend_Cache_Backend_TwoLevels extends IfwZend_Cache_Backend implements IfwZend_Cache_Backend_ExtendedInterface
{
    /**
     * Available options
     *
     * =====> (string) slow_backend :
     * - Slow backend name
     * - Must implement the IfwZend_Cache_Backend_ExtendedInterface
     ...
     * - See IfwZend_Cache::factory() method
     *
     ...
     * Slow Backend
     *
     * @var IfwZend_Cache_Backend_ExtendedInterface
     */
    ...
    public function __construct(array $options = array())
    {
        ...
        if ($this->_options['slow_backend'] === null) {
            IfwZend_Cache::throwException('slow_backend option has to set');
        } elseif ($this->_options['slow_backend'] instanceof IfwZend_Cache_Backend_ExtendedInterface) {
            $this->_slowBackend = $this->_options['slow_backend'];
        } else {
            $this->_slowBackend = IfwZend_Cache::_makeBackend(
            ...
            if (!in_array('IfwZend_Cache_Backend_ExtendedInterface', class_implements($this->_slowBackend))) {
                IfwZend_Cache::throwException('slow_backend must implement the IfwZend_Cache_Backend_ExtendedInterface interface');
            }
        }

        if ($this->_options['fast_backend'] === null) {
            IfwZend_Cache::throwException('fast_backend option has to set');
        } elseif ($this->_options['fast_backend'] instanceof IfwZend_Cache_Backend_ExtendedInterface) {
            $this->_fastBackend = $this->_options['fast_backend'];
        } else {
            $this->_fastBackend = IfwZend_Cache::_makeBackend(
            ...
            if (!in_array('IfwZend_Cache_Backend_ExtendedInterface', class_implements($this->_fastBackend))) {
                IfwZend_Cache::throwException('fast_backend must implement the IfwZend_Cache_Backend_ExtendedInterface interface');
            }
        }
     ...
     *
     * Available modes are :
     * IfwZend_Cache::CLEANING_MODE_ALL (default)    => remove all cache entries ($tags is not used)
     * IfwZend_Cache::CLEANING_MODE_OLD              => remove too old cache entries ($tags is not used)
     * IfwZend_Cache::CLEANING_MODE_MATCHING_TAG     => remove cache entries matching all given tags
     *                                               ($tags can be an array of strings or a single string)
     * IfwZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG => remove cache entries not {matching one of the given tags}
     *                                               ($tags can be an array of strings or a single string)
     * IfwZend_Cache::CLEANING_MODE_MATCHING_ANY_TAG => remove cache entries matching any given tags
     *                                               ($tags can be an array of strings or a single string)
     *
     ...
     * @return boolean true if no problem
     */
    public function clean($mode = IfwZend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        switch($mode) {
            case IfwZend_Cache::CLEANING_MODE_ALL:
                $boolFast = $this->_fastBackend->clean(IfwZend_Cache::CLEANING_MODE_ALL);
                $boolSlow = $this->_slowBackend->clean(IfwZend_Cache::CLEANING_MODE_ALL);
                return $boolFast && $boolSlow;
                break;
            case IfwZend_Cache::CLEANING_MODE_OLD:
                return $this->_slowBackend->clean(IfwZend_Cache::CLEANING_MODE_OLD);
            case IfwZend_Cache::CLEANING_MODE_MATCHING_TAG:
                ...
                return $res;
                break;
            case IfwZend_Cache::CLEANING_MODE_NOT_MATCHING_TAG:
                ...
                return $res;
                break;
            case IfwZend_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                ...
                break;
            default:
                IfwZend_Cache::throwException('Invalid mode for clean() method');
                break;
        }
    }
