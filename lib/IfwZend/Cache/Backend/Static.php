 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Cache_Backend_Static
    extends IfwZend_Cache_Backend
    implements IfwZend_Cache_Backend_Interface
{
     ...
     * @param  string $name
     * @param  mixed $value
     * @return IfwZend_Cache_Backend_Static
     */
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if (!$this->_verifyPath($id)) {
            IfwZend_Cache::throwException('Invalid cache id: does not match expected public_dir path');
        }
        if ($doNotTestCacheValidity) {
            $this->_log("IfwZend_Cache_Backend_Static::load() : \$doNotTestCacheValidity=true is unsupported by the Static backend");
        }
    ...
    protected function _createDirectoriesFor($path)
    {
        if (!is_dir($path)) {
            ...
            if ( !@mkdir($path, $this->_octdec($this->_options['cache_directory_umask']), true)) {
                ...
                umask($oldUmask);
                IfwZend_Cache::throwException("Can't create directory: {$lastErr['message']}");
            }
            umask($oldUmask);
        }
    }
    ...
    public function clean($mode = IfwZend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        switch ($mode) {
            ...
            case IfwZend_Cache::CLEANING_MODE_MATCHING_ANY_TAG:
                if (empty($tags)) {
                    throw new IfwZend_Exception('Cannot use tag matching modes as no tags were defined');
                }
                if ($this->_tagged === null && $tagged = $this->getInnerCache()->load(self::INNER_CACHE_NAME)) {
                    $this->_tagged = $tagged;
                } elseif (!$this->_tagged) {
            ...
            case IfwZend_Cache::CLEANING_MODE_OLD:
                $this->_log("IfwZend_Cache_Backend_Static : Selected Cleaning Mode Currently Unsupported By This Backend");
                break;
     ...
     *
     * @param  IfwZend_Cache_Core
     * @return void
     */
    public function setInnerCache(IfwZend_Cache_Core $cache)
    {
    ...
    public function getInnerCache()
    {
        if ($this->_tagCache === null) {
            IfwZend_Cache::throwException('An Inner Cache has not been set; use setInnerCache()');
        }
    ...
    protected static function _validateIdOrTag($string)
    {
        ...
        if (!preg_match(
            ...
            IfwZend_Cache::throwException("Invalid id or tag '$string' : must be a valid URL path");
        }
    }
