 ...
 * @subpackage IfwZend_Cache_Frontend
...
/**
 * @see IfwZend_Cache_Core
 */
//require_once 'IfwZend/Cache/Core.php';
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Cache_Frontend_Page extends IfwZend_Cache_Core
{
    /**
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_specificOptions['http_conditional'])) {
            if ($this->_specificOptions['http_conditional']) {
                IfwZend_Cache::throwException('http_conditional is not implemented for the moment !');
            }
        }
    ...
    {
        if (!is_array($options)) {
            IfwZend_Cache::throwException('default_options must be an array !');
        }
        foreach ($options as $key=>$value) {
            if (!is_string($key)) {
                IfwZend_Cache::throwException("invalid option [$key] !");
            }
            $key = strtolower($key);
    ...
    {
        if (!is_array($regexps)) {
            IfwZend_Cache::throwException('regexps option must be an array !');
        }
        foreach ($regexps as $regexp=>$conf) {
            if (!is_array($conf)) {
                IfwZend_Cache::throwException('regexps option must be an array of arrays !');
            }
            $validKeys = array_keys($this->_specificOptions['default_options']);
            foreach ($conf as $key=>$value) {
                if (!is_string($key)) {
                    IfwZend_Cache::throwException("unknown option [$key] !");
                }
                $key = strtolower($key);
