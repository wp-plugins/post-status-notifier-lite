 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class IfwZend_Cache_Frontend_Function extends IfwZend_Cache_Core
{
    /**
    ...
    public function call($callback, array $parameters = array(), $tags = array(), $specificLifetime = false, $priority = 8)
    {
        if (!is_callable($callback, true, $name)) {
            IfwZend_Cache::throwException('Invalid callback');
        }
    ...
    public function makeId($callback, array $args = array())
    {
        ...
        if (isset($object)) {
            try {
                ...
                IfwZend_Cache::throwException($e->getMessage());
            }
            if (!$tmp) {
                $lastErr = error_get_last();
                IfwZend_Cache::throwException("Can't serialize callback object to generate id: {$lastErr['message']}");
            }
            $name.= '__' . $tmp;
        }
        ...
        if ($args) {
            ...
            if (!$argsStr) {
                $lastErr = error_get_last();
                throw IfwZend_Cache::throwException("Can't serialize arguments to generate id: {$lastErr['message']}");
            }
        }
