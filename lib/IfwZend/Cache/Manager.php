...
class IfwZend_Cache_Manager
{
     ...
     *
     * @param  string $name
     * @param  IfwZend_Cache_Core $cache
     * @return IfwZend_Cache_Manager
     */
    public function setCache($name, IfwZend_Cache_Core $cache)
    {
    ...
    public function getCache($name)
    {
        ...
        if (isset($this->_optionTemplates[$name])) {
            if ($name == self::PAGECACHE
                ...
                || !$this->_optionTemplates[$name]['backend']['options']['tag_cache'] instanceof IfwZend_Cache_Core)
            ) {
                $this->_optionTemplates[$name]['backend']['options']['tag_cache']
                    = $this->getCache(self::PAGETAGCACHE);
            }

            $this->_caches[$name] = IfwZend_Cache::factory(
    ...
    public function setCacheTemplate($name, $options)
    {
        if ($options instanceof IfwZend_Config) {
            ...
            //require_once 'IfwZend/Cache/Exception.php';
            throw new IfwZend_Cache_Exception('Options passed must be in'
                . ' an associative array or instance of IfwZend_Config');
        }
     ...
     * @param  array $options
     * @return IfwZend_Cache_Manager
     * @throws IfwZend_Cache_Exception for invalid options format or if option templates do not have $name
     */
    public function setTemplateOptions($name, $options)
    {
        ...
        if (!isset($this->_optionTemplates[$name])) {
            throw new IfwZend_Cache_Exception('A cache configuration template'
