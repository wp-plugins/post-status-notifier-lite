    ...
    public static function init($config)
    {
        if (!self::$_is_init) {
            ...
            if ((int)$config->use_pdo === 1) {
                Ifw_Wp_ORM_ORM::configure('use_pdo', true);
                Ifw_Wp_ORM_ORM::configure(sprintf('mysql:host=%s;dbname=%s', DB_HOST, DB_NAME));
                Ifw_Wp_ORM_ORM::configure('username', DB_USER);
                Ifw_Wp_ORM_ORM::configure('password', DB_PASSWORD);
                // UTF8 setting, see: https://github.com/j4mie/idiorm/issues/2
                Ifw_Wp_ORM_ORM::configure('driver_options', array(
