...
class Ifw_Wp_CustomPostType 
{
    ...
    {
        Ifw_Wp_Proxy_Action::addInit(array($this, '_register'));

        if (method_exists($this, 'getCustomColums')) {
            Ifw_Wp_Proxy_Action::add('manage_'. $this->getId() .'_posts_columns', array($this, 'getCustomColums'));
            //add_filter('manage_'. $this->getId() .'_posts_columns', array($this, 'getCustomColums'));
        }
        if (method_exists($this, 'getColumsContent')) {
            Ifw_Wp_Proxy_Action::add('manage_'. $this->getId() .'_posts_custom_column', array($this, 'getColumsContent'), 10, 2);
        ...
        if (method_exists($this, 'getSortableColumns')) {
            Ifw_Wp_Proxy_Action::add('manage_edit-'. $this->getId() .'_sortable_columns', array($this, 'getSortableColumns'));
            //add_filter('manage_edit-'. $this->getId() .'_sortable_columns', array($this, 'getSortableColumns'));
        }
        if (method_exists($this, 'addToFeed')) {
            Ifw_Wp_Proxy_Action::add('request', array($this, 'addToFeed'));
            //add_filter('request', array($this, 'addToFeed'));
        }
        if (method_exists($this, 'restrictManagePosts')) {
            Ifw_Wp_Proxy_Action::add('restrict_manage_posts', array($this, 'restrictManagePosts'));
