 ...
 * </pre>
 */
class IfwTwig_TokenParser_Macro extends IfwTwig_TokenParser
{
    /**
    ...
    public function parse(IfwTwig_Token $token)
    {
        ...
        if ($stream->test(IfwTwig_Token::NAME_TYPE)) {
            ...
            if ($value != $name) {
                throw new IfwTwig_Error_Syntax(sprintf("Expected endmacro for macro '$name' (but %s given)", $value), $stream->getCurrent()->getLine(), $stream->getFilename());
            }
        }
