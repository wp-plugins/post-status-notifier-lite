 ...
 */
class IfwTwig_TokenParser_Sandbox extends IfwTwig_TokenParser
{
    /**
    ...
    public function parse(IfwTwig_Token $token)
    {
        ...
        if (!$body instanceof IfwTwig_Node_Include) {
            foreach ($body as $node) {
                if ($node instanceof IfwTwig_Node_Text && ctype_space($node->getAttribute('data'))) {
                    continue;
                }

                if (!$node instanceof IfwTwig_Node_Include) {
                    throw new IfwTwig_Error_Syntax('Only "include" tags are allowed within a "sandbox" section', $node->getLine(), $this->parser->getFilename());
                }
            }
        }

        return new IfwTwig_Node_Sandbox($body, $token->getLine(), $this->getTag());
    }
